...
export function ShoppingScene({ scene, config }: ShoppingSceneProps) {
  const staggerDelay = (scene.animation.stagger / 1000) / config.speed;

  return (
    <div className="flex flex-col items-center justify-center h-full p-8 bg-gradient-to-br from-green-50 to-emerald-50">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      {scene.servicesReceived && (
        ...
        >
          ...
          {scene.servicesReceived.map((service, index) => (
            <div
              key={index}
              className="bg-blue-50 rounded-lg p-3 border-l-4"
