...
export function WorkScene({ scene, config }: WorkSceneProps) {
  const staggerDelay = (scene.animation.stagger / 1000) / config.speed;

  return (
    <div className="flex flex-col items-center justify-center h-full p-8 bg-gradient-to-br from-indigo-50 to-blue-50">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <p className="text-sm text-gray-600 mb-1">Coût du travail (brut + patronales)</p>
