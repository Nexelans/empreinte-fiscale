...
interface CommuteSceneProps {
  scene: Scene;
  config: AnimationConfig;
  isPlaying: boolean;
}

export function CommuteScene({ scene, config, isPlaying }: CommuteSceneProps) {
  const staggerDelay = (scene.animation.stagger / 1000) / config.speed;

  return (
    <div className="flex flex-col items-center justify-center h-full p-8 bg-gradient-to-br from-blue-50 to-cyan-50">
      <motion.div
      ...
      {scene.servicesReceived && (
        ...
        >
          ...
          {scene.servicesReceived.map((service, index) => (
            <div
              key={index}
              className="bg-purple-50 rounded-lg p-3 border-l-4"
